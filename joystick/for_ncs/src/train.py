# Before run this file, remember to put the folder with the training image data into 'images' folder,
# and put test.txt and train.txt into 'splits' folder
# To do the training, please run 'source environment.sh' and 'python setting.py' before
# Then run python train.py, and it should start the training process. 
...
def create_lmdbs(split_num, cwd):
    trainfile = os.path.join(cwd, "../splits/train.txt")#.format(split_num))
    ...
    call([lmdb_tool_path,  "--resize_height=101", "--resize_width=101", "../images/", trainfile, train_lmdb_path])
    call([lmdb_tool_path,   "--resize_height=101", "--resize_width=101", "../images/", testfile, test_lmdb_path])
...
def train_split(split_num, cwd, solverpath):
    ...
    call([caffe_path, "train", "-solver", solverpath, "-gpu", str(GPU)])
...
def evaluate_results(split):
    ...
    call(["./CaffeNetAnalysis/CaffeNetAnalysisMain",
          "../splits/test.txt",#.format(split)
          os.path.join(os.path.abspath("../images/"), ""),
          "../caffe_data/deploy.prototxt",
          "../results/{0}/snapshots/solvertemplate_iter_100000.caffemodel".format(split),
          "../classid.txt",
...
if __name__ == "__main__":
    for i in range(1):  # train the 1 splits
        cwd = os.getcwd()
